...
var handleDuplicateFieldsDB = function handleDuplicateFieldsDB(err) {
  // console.log(err);
  var value = err.message.match(/(["'])(\\?.)*?\1/)[0]; // console.log(value);

  var message = "Duplicate field value: ".concat(value, ". Please use another value!"); // console.log(message);
...
var sendErrorProd = function sendErrorProd(err, req, res) {
  ...
  if (req.originalUrl.startsWith('/api')) {
    ...
    console.log(err); // B) Programming or other unknown error: don't leak error details
    // 1) Log error
...
module.exports = function (err, req, res, next) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (error.name === 'CastError') error = handleCastErrorDB(error);else if (error.code === 11000) error = handleDuplicateFieldsDB(error);else if (error.name === 'ValidationError') error = handleValidationErrorDB(error);else if (error.name === 'JsonWebTokenError') error = handleJWTError();else if (error.name === 'TokenExpiredError') error = handleJWTExpiredError();
